...
def task_eo():
    return {
        "actions": [
            cmd("wget -O eo https://hg.sr.ht/~duangle/majoreo/raw/eo?rev=tip"),
            cmd("chmod +x eo"),
        ],
        "targets": ["eo"],
        "uptodate": [True],
...
def task_force_bootstrap():
    return {
        "verbosity": 2,
        "actions": [
            cmd("rm -rf .eo lib include"),
            cmd("./eo init && touch ./.eo/initialized"),
            cmd("./eo install -y bootstrap"),
        ],
        "file_dep": ["eo"],
        "uptodate": [False],
    }
