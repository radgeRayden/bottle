...
static char __glsl_shader_vert_spv[] =
R"s(
@group(0)
@binding(0)
...
fn main(
   ...
   out.Color = aColor;
   out.UV = aUV;
   out.position = transform * vec4<f32>(aPos, 0., 1.);
   return out;
}
)s";
...
static char __glsl_shader_frag_spv[] =
R"s(
struct VertexOutput {
   @location(0) Color: vec4<f32>,
   @location(1) UV: vec2<f32>,
...
fn main(vertex: VertexOutput) -> @location(0) vec4<f32> {
   return vertex.Color * textureSample(t, s, vertex.UV.xy);
}
)s";
...
static WGPUProgrammableStageDescriptor ImGui_ImplWGPU_CreateShaderModule(char* const binary_data, WGPUShaderStage stage)
{
    WGPUShaderModuleWGSLDescriptor wgsl_desc = {};
    wgsl_desc.chain.sType = WGPUSType_ShaderModuleWGSLDescriptor;
    wgsl_desc.code = binary_data;
...
bool ImGui_ImplWGPU_CreateDeviceObjects()
{
    ...
    WGPUBindGroupLayout bg_layouts[] = {{}, {}};
    ...
    entries[0].buffer = {nullptr, WGPUBufferBindingType_Uniform};

    entries[1].binding = 1;
    entries[1].visibility = WGPUShaderStage_Vertex | WGPUShaderStage_Fragment;
    entries[1].sampler = {nullptr, WGPUSamplerBindingType_Filtering};

    bg_layout_desc.entries = entries;
    bg_layouts[0] = wgpuDeviceCreateBindGroupLayout(g_wgpuDevice, &bg_layout_desc);

    bg_layout_desc.entryCount = 1;
    entries[0] = {};
    entries[0].binding = 0;
    entries[0].visibility = WGPUShaderStage_Vertex | WGPUShaderStage_Fragment;
    entries[0].texture = {nullptr, WGPUTextureSampleType_Float, WGPUTextureViewDimension_2D};
    bg_layout_desc.entries = entries;
    bg_layouts[1] = wgpuDeviceCreateBindGroupLayout(g_wgpuDevice, &bg_layout_desc);

    WGPUPipelineLayoutDescriptor pip_layout_desc = {};
    pip_layout_desc.label = "imgui pipeline layout";
    pip_layout_desc.bindGroupLayoutCount = 2;
    pip_layout_desc.bindGroupLayouts = bg_layouts;
    graphics_pipeline_desc.layout = wgpuDeviceCreatePipelineLayout(g_wgpuDevice, &pip_layout_desc);

    // Create the vertex shader
    WGPUProgrammableStageDescriptor vertex_shader_desc = ImGui_ImplWGPU_CreateShaderModule(__glsl_shader_vert_spv, WGPUShaderStage_Vertex);
    ...
    WGPUProgrammableStageDescriptor pixel_shader_desc = ImGui_ImplWGPU_CreateShaderModule(__glsl_shader_frag_spv, WGPUShaderStage_Fragment);
